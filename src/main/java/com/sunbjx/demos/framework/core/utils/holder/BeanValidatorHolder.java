...
public class BeanValidatorHolder implements InitializingBean {
     ...
     * 验证失败将抛出异常
     */
    public static final <T> void validateWithException(T object, Class<?>... groups)
     ...
     * 验证失败将抛出异常
     */
    public static final <T> void validatePropertyWithException(T object, String propertyName, Class<?>... groups)
            throws ConstraintViolationException {
        Set constraintViolations = getRequiredValidator().validateProperty(object, propertyName);
        String
                msg =
                "validate property failure on object:" + object.getClass().getSimpleName() + "." + propertyName + "";
        if (!constraintViolations.isEmpty()) {
            throw new ConstraintViolationException(msg, constraintViolations);
        }
    }
     ...
     * 验证失败将抛出异常
     */
    public static final <T> void validateValueWithException(Class<T> beanType, String propertyName, Object value,
        ...
        String
                msg =
                "validate value failure on object:" + beanType.getSimpleName() + "." + propertyName + " value:" + value;
        if (!constraintViolations.isEmpty()) {
            throw new ConstraintViolationException(msg, constraintViolations);
        }
    }
