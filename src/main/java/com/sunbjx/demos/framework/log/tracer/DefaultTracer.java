...
package com.sunbjx.demos.framework.log.tracer;




import com.sunbjx.demos.framework.log.*;
import com.sunbjx.demos.framework.log.log.SpanLogger;
import com.sunbjx.demos.framework.log.util.ExceptionUtils;
...
public class DefaultTracer implements Tracer {
	...
	public DefaultTracer(Random random, SpanNamer spanNamer, SpanLogger spanLogger) {
		this.random = random;
	...
	public Span close(Span span) {
		...
		if (!span.equals(cur)) {
			ExceptionUtils
					.warn("Tried to close span but it is not the current span: " + span + ".  You may have forgotten to close or detach "
						  + cur);
		} else {
			span.stop();
