...
public class ASMUtils {
     ...
     * 缓存ConstructorAccess
     */
    private static ConcurrentMap<Class<?>, ConstructorAccess<?>>
            constructorAccessCache
            = new ConcurrentHashMap<Class<?>, ConstructorAccess<?>>();
    ...
    public <T> ConstructorAccess<T> createConstructorAccess(Class<T> clazz) {
        @SuppressWarnings("unchecked") ConstructorAccess<T> access = (ConstructorAccess<T>) constructorAccessCache.get(clazz);
        if (access == null) {
